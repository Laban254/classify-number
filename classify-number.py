...
async def classify_number(number: int = Query(...)):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            fun_fact_response = await client.get(f"http://numbersapi.com/{number}")
...
# Custom error handler for validation errors (invalid inputs)
...
async def validation_exception_handler(request, exc):
    ...
    return JSONResponse(
        status_code=400,
        content={
            "number": sanitized_param_value,
            "error": True
        }
    )

# Error handling for 400 Bad Request (catching general HTTPExceptions)
@app.exception_handler(HTTPException)
async def bad_request_handler(request, exc):
    if exc.status_code == 400:
        sanitized_param_value = str(request.query_params.get("number", "")).strip()
        return JSONResponse(
